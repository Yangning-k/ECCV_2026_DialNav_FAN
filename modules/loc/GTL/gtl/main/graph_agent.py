...
class GraphVlnAgent(Seq2SeqAgent):
    ...
    def _build_model(self):
        ...
        self.scene_cache_max = int(os.environ.get("GTL_SCENE_CACHE_MAX", "2"))
        self.pano_feat_cache_order = []
        self.pano_feat_cache_max = int(os.environ.get("GTL_PANO_CACHE_MAX", "500"))
        self.scene_cache_dir = os.environ.get("GTL_SCENE_CACHE_DIR", "")
        if self.scene_cache_dir:
            # Scene embeddings depend on the localization checkpoint.  The
            # Guide-side E1 experiment creates multiple GTL agents in one
            # process, so a scan-only cache path would silently let one
            # checkpoint consume another checkpoint's embeddings.
            resume_file = str(
                getattr(self.args, "resume_file", None) or "default"
            )
            cache_namespace = hashlib.sha1(
                resume_file.encode("utf-8")
            ).hexdigest()[:12]
            self.scene_cache_dir = os.path.join(
                self.scene_cache_dir, cache_namespace)
        if self.scene_cache_dir:
            os.makedirs(self.scene_cache_dir, exist_ok=True)
        # panorama feature cache
        self.pano_feat_cache = {}
        self.local_embed_cache = {}
        self.profile_timing = (
            defaultdict(float)
    ...
    def _remember_pano_cache(self, scan_vp, features):
        self.pano_feat_cache[scan_vp] = features
        if scan_vp in self.pano_feat_cache_order:
            self.pano_feat_cache_order.remove(scan_vp)
        self.pano_feat_cache_order.append(scan_vp)
        while len(self.pano_feat_cache_order) > self.pano_feat_cache_max:
            old_scan_vp = self.pano_feat_cache_order.pop(0)
            self.pano_feat_cache.pop(old_scan_vp, None)

    def _scene_cache_path(self, scan_id):
        if not self.scene_cache_dir:
            return None
        filename = str(scan_id).replace(os.sep, "_") + ".pt"
        return os.path.join(self.scene_cache_dir, filename)

    def _get_pair_dist_matrix(self, gmap):
        pair_dist = getattr(gmap, "_pair_dist_matrix", None)
        if pair_dist is not None:
            return pair_dist
        nodes = list(gmap.node_positions.keys())
        pair_dist = np.zeros((len(nodes), len(nodes)), dtype=np.float32)
        for i in range(1, len(nodes)):
            for j in range(i + 1, len(nodes)):
                pair_dist[i, j] = pair_dist[j, i] = gmap.graph.distance(
                    nodes[i], nodes[j])
        gmap._pair_dist_matrix = torch.from_numpy(pair_dist)
        return gmap._pair_dist_matrix

    def _load_scene_cache(self, scan_id, start_vp):
        path = self._scene_cache_path(scan_id)
        ...
        try:
            ...
            if payload.get("format") == 1:
                ...
                gmap = GraphMap(start_vp)
                gmap.node_positions = dict(
                    zip(nodes, payload["positions"]))
                gmap.node_embeds = {
                    vp: [embeds[index], 1]
                    for index, vp in enumerate(nodes)
                }
                gmap._pair_dist_matrix = payload["pair_dist"]
                return gmap

            # Older runs stored the same fully embedded graph under the
            # GraphMap attribute names.  Reusing it avoids rebuilding every
            # val_seen scene after the cache format was tightened to include
            # an explicit format marker.  Do not rewrite the old file while
            # another evaluation may be reading the shared cache.
            legacy_keys = {
                "node_positions",
                "node_embeds",
                "graph_dis",
            ...
            if not legacy_keys.issubset(payload):
                return None
            gmap = GraphMap(start_vp)
            gmap.node_positions = dict(payload["node_positions"])
            gmap.node_embeds = dict(payload["node_embeds"])
            for source, targets in payload["graph_dis"].items():
                gmap.graph._dis[source].update(targets)
            for source, targets in payload["graph_point"].items():
                gmap.graph._point[source].update(targets)
            gmap.graph._visited = set(payload["graph_visited"])
            gmap.node_step_ids = dict(payload.get("node_step_ids", {}))
            gmap._pair_dist_matrix = self._get_pair_dist_matrix(gmap)
            return gmap
        except Exception as exc:
            print(f"scene cache load skipped for {scan_id}: {exc}")
            return None

    def _save_scene_cache(self, scan_id, gmap):
        path = self._scene_cache_path(scan_id)
        if path is None:
            return
        nodes = list(gmap.node_positions.keys())
        embeds = torch.stack([
            gmap.get_node_embed(vp).detach().cpu()
            for vp in nodes
        ])
        payload = {
            "format": 1,
            "nodes": nodes,
            "positions": [gmap.node_positions[vp] for vp in nodes],
            "embeds": embeds,
            "pair_dist": self._get_pair_dist_matrix(gmap),
        }
        temp_path = f"{path}.tmp.{os.getpid()}"
        torch.save(payload, temp_path)
        os.replace(temp_path, path)

    def _load_or_build_scene(self, scan_id, start_vp):
        path = self._scene_cache_path(scan_id)
        if path is None:
            gmap = GraphMap(start_vp)
            self.build_map_embeddings(scan_id, gmap, batch_size=1024)
            return gmap
        profile_start = time.perf_counter()
        lock_path = f"{path}.lock"
        with open(lock_path, "a") as lock_file:
            fcntl.flock(lock_file, fcntl.LOCK_EX)
            gmap = self._load_scene_cache(scan_id, start_vp)
            if gmap is not None:
                if self.profile_timing is not None:
                    self.profile_timing["scene_cache_hits"] += 1
                    self.profile_timing["scene_cache_load_s"] += (
                        time.perf_counter() - profile_start
                    )
                return gmap
            gmap = GraphMap(start_vp)
            self.build_map_embeddings(scan_id, gmap, batch_size=1024)
            self._save_scene_cache(scan_id, gmap)
            if self.profile_timing is not None:
                self.profile_timing["scene_builds"] += 1
                self.profile_timing["scene_build_s"] += (
                    time.perf_counter() - profile_start
                )
            return gmap
    ...
    def _panorama_feature_variable(self, obs):
        ...
        for i, ob in enumerate(obs):
            ...
            if scan_vp in self.pano_feat_cache:
                ...
                self._remember_pano_cache(scan_vp, cached_feats)
                ...
                self._remember_pano_cache(scan_vp, {
    ...
    def _loc_gmap_variable(self, gmaps):
        ...
        for i, gmap in enumerate(gmaps):
            ...
            if not gmap_img_embeds.is_cuda:
                gmap_img_embeds = gmap_img_embeds.cuda()

            # gmap_pos_fts = gmap.get_pos_fts(
            #     obs[i]['viewpoint'], gmap_vpids, obs[i]['heading'], obs[i]['elevation'],
            # )

            gmap_pair_dists = self._get_pair_dist_matrix(gmap)

            batch_gmap_img_embeds.append(gmap_img_embeds)
            batch_gmap_step_ids.append(torch.LongTensor(gmap_step_ids))
            # batch_gmap_pos_fts.append(torch.from_numpy(gmap_pos_fts))
            batch_gmap_pair_dists.append(gmap_pair_dists)
    ...
    def test(self, batch_items, cache_graph_enc=False, save_gmap_path=None):
        ...
        self.last_loc_nodes = [
            list(gmap.node_positions.keys()) for gmap in gmaps]
        self.last_loc_conf = torch.softmax(
            logits, dim=1).max(1)[0].detach().cpu().tolist()
    ...
    def localize(self, texts, scanIds):
        profile_start = (
            time.perf_counter() if self.profile_timing is not None else None
        )
        ...
        if self.profile_timing is not None:
            ...
            self.profile_timing["localize_prepare_s"] += prepare_elapsed
        nav_inputs = self._loc_gmap_variable(gmaps)
        nav_inputs.update({
            'txt_embeds': txt_embeds,
            'txt_masks': language_inputs['txt_masks'],
        ...
        loc_outs = self.vln_bert('localize', nav_inputs)
        if self.profile_timing is not None:
            self.profile_timing["localize_forward_s"] += (
        ...
        if self.profile_timing is not None:
            self.profile_timing["localize_calls"] += 1
            self.profile_timing["localize_s"] += (
                time.perf_counter() - profile_start
            )
        return predicted

    @torch.no_grad()
    def build_local_map_embeddings(self, scan_id, gmap, batch_size):
        profile_start = (
            time.perf_counter() if self.profile_timing is not None else None
        )
        all_viewpoints = list(gmap.node_positions.keys())
        pending = []
        for vp in all_viewpoints:
            key = (scan_id, vp)
            if key in self.local_embed_cache:
                embed = self.local_embed_cache[key]
                if not embed.is_cuda:
                    embed = embed.cuda()
                gmap.update_node_embed(
                    vp, embed, rewrite=True)
            else:
                pending.append(vp)
        for i in range(0, len(pending), batch_size):
            batch_vps = pending[i:i + batch_size]
            self.env.env._init_sims(len(batch_vps))
            self.env.env.newEpisodes(
                [scan_id] * len(batch_vps), batch_vps, [3.14] * len(batch_vps))
            ...
            for j, vp in enumerate(batch_vps):
                gmap.update_node_embed(vp, avg_pano_embeds[j], rewrite=True)
                self.local_embed_cache[(scan_id, vp)] = (
                    avg_pano_embeds[j].detach().cpu())
        if self.profile_timing is not None:
            self.profile_timing["local_embedding_s"] += (
                time.perf_counter() - profile_start
            )
        return gmap

    @torch.no_grad()
    def localize_local(self, obs, texts):
        profile_start = (
            time.perf_counter() if self.profile_timing is not None else None
        )
        encoded_instructions = [
            self.tokenizer.encode(
                text, max_length=self.args.max_dialog_len,
        ...
        for ob in obs:
            ...
            if os.environ.get("LOCAL_ANS_WIDEN", "0") == "1":
                # The widened pool is limited to nodes the Navigator has
                # already visited plus candidates visible at the current
                # viewpoint.  No unvisited scene node is introduced here.
                current = ob['viewpoint']
                current_candidates = [
                    candidate['viewpointId']
                    for candidate in ob.get('candidate', [])
                ]
                visited = ob.get('_observed_viewpoints', [])
                allowed = set([current, *current_candidates, *visited])
                positions = ob.get('_observed_positions', {})
                for viewpoint in visited:
                    if viewpoint in allowed and viewpoint in positions:
                        gmap.node_positions[viewpoint] = positions[viewpoint]
                for source, target, distance in ob.get(
                        '_observed_edges', []):
                    if source in allowed and target in allowed:
                        gmap.graph.add_edge(source, target, distance)
                for viewpoint in gmap.node_positions:
                    if viewpoint in gmap.node_positions:
                        gmap.graph.update(viewpoint)
            self.build_local_map_embeddings(
                ob['scan'], gmap, batch_size=len(gmap.node_positions))
            gmaps.append(gmap)
        ...
        for i, gmap in enumerate(gmaps):
            ...
            predicted.append(nodes[max_idx[i]])
        if self.profile_timing is not None:
            self.profile_timing["localize_local_calls"] += 1
            self.profile_timing["localize_local_s"] += (
                time.perf_counter() - profile_start
            )
        return predicted

    def train_local_step(self, batch_items):
        scans = [item.get('scanName') or item['scan'] for item in batch_items]
        current_vps = [item['current_pano'] for item in batch_items]
        self.env.env._init_sims(len(batch_items))
        self.env.env.newEpisodes(
            scans, current_vps, [3.14] * len(batch_items))
        obs_batch = self.env._get_obs()

        valid_items = []
        valid_obs = []
        for item, ob in zip(batch_items, obs_batch):
            target = item['target_pano']
            if target in [ob['viewpoint']] + [
                    c['viewpointId'] for c in ob['candidate']]:
                valid_items.append({
                    **item,
                    'viewpointId': target,
                    'scanName': item.get('scanName') or item['scan'],
                })
                valid_obs.append(ob)
        if not valid_items:
            return 0.0, [], []
        ...
        encoded_instructions = [
            self.tokenizer.encode(
                item['text'], max_length=self.args.max_dialog_len,
                padding='max_length', truncation=True)
        ...
        logits = self.vln_bert('localize', nav_inputs)['global_logits']
        labels = torch.tensor([
            list(gmap.node_positions.keys()).index(item['viewpointId'])
            for gmap, item in zip(gmaps, valid_items)
        ...
        _, max_idx = torch.max(logits.detach(), dim=1)
        predicted = [
            list(gmap.node_positions.keys())[int(max_idx[i])]
            for i, gmap in enumerate(gmaps)
        ]
        targets = [item['viewpointId'] for item in valid_items]
        return loss.item(), predicted, targets
    ...
    def prepare_gmaps(self, batch_items):
        batch_scans = [item.get('scanName') or item['scan'] for item in batch_items]
        batch_viewpoints = [item['viewpointId'] for item in batch_items]
        gmaps = []
        if self.scene_cache_dir:
            for scan, vp in zip(batch_scans, batch_viewpoints):
                if scan in self.scene_gmap_cache:
                    gmap = self.scene_gmap_cache[scan]
                else:
                    gmap = self._load_or_build_scene(scan, vp)
            ...
            for scan, vp in zip(batch_scans, batch_viewpoints):
                if scan in self.scene_gmap_cache:
                    ...
                    pending_gmaps.append(gmap)
                gmaps.append(gmap)
            if pending_gmaps:
                built_gmaps = self.build_all_map_embeddings(
                    pending_scans, pending_gmaps, batch_size=1024)
                for scan, gmap in zip(pending_scans, built_gmaps):
                    self.scene_gmap_cache[scan] = gmap
                    if scan in self.scene_cache_order:
                        self.scene_cache_order.remove(scan)
                    self.scene_cache_order.append(scan)
        gmaps = [
            self.scene_gmap_cache[scan]
            for scan in batch_scans
        ]
        while len(self.scene_cache_order) > self.scene_cache_max:
            old_scan = self.scene_cache_order.pop(0)
            self.scene_gmap_cache.pop(old_scan, None)
            for scan_vp in list(self.pano_feat_cache):
                if scan_vp.startswith(f"{old_scan}_"):
                    self.pano_feat_cache.pop(scan_vp, None)
                    if scan_vp in self.pano_feat_cache_order:
                        self.pano_feat_cache_order.remove(scan_vp)
            gc.collect()
    ...
    def build_map_embeddings(self, scan_id, gmap: GraphMap, batch_size):
        ...
        for i in range(0, len(all_viewpoints), batch_size):
            ...
            if self.vln_bert.training:
                pano_embeds, pano_masks = self.vln_bert(
                    'panorama', pano_inputs)
            else:
                with torch.no_grad():
                    pano_embeds, pano_masks = self.vln_bert(
                        'panorama', pano_inputs)
